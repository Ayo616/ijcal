...
def save_result(results, args):
    if args.dataset in ('cora', 'amazon-photo', 'coauthor-cs','wiki-cs','actor','webkb'):
...
def save_cust_result(results, args):
    filename = f'results/{args}.csv'
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(results)
        write_obj.write(f'\n')


def save_cust_result2(results, args):
    filename = f'results/myresults/{args}.csv'
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(results)
        write_obj.write(f'\n')
...
# 分布内数据能量计算箱线图，纯模式
def save_boxplot_ind(ind_result,args):
    ...
    results = min_val + ',' + q1 + ',' + q2 + ',' + q3 + ',' + max_val
...
# 分布外数据能量计算箱线图，纯模式
def save_boxplot_ood(ood_result, args):
...
def save_in_out_max_min(ind_result,ood_result,args):
    results = np.max(ind_result.tolist()).astype(str)+','+ np.min(ind_result.tolist()).astype(str) +',' + np.max(ood_result.tolist()).astype(str) + ',' +np.min(ood_result.tolist()).astype(str)
    filename = f'results/myresults/{args}.csv'
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(results)
        write_obj.write(f'\n')
