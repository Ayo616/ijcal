...
from parse import parser_add_main_args,my_parser_add_main_args
...
# my_parser_add_main_args(parser)
...
if args.method == 'msp':
    model = nn.DataParallel(MSP(d, c, args)).to(device)
elif args.method in 'gnnsafe':
    model = nn.DataParallel(GNNSafe(d, c, args)).to(device)
elif args.method == 'OE':
    model = nn.DataParallel(OE(d, c, args)).to(device)
elif args.method == "ODIN":
    model = nn.DataParallel(ODIN(d, c, args)).to(device)
elif args.method == "Mahalanobis":
    model = nn.DataParallel(Mahalanobis(d, c, args)).to(device)
